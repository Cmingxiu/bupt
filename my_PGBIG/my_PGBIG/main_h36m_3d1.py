...
from model import model_trans as model
...
def main(opt):
    ...
    # evaluation
    if opt.is_eval:
        ret_test, _ = run_model( net_pred, is_train=3, data_loader=test_loader, opt=opt)
        ret_log = np.array([])
        head = np.array([])
...
def eval(opt):
    ...
    for act in acts:
        ret_test, out_seq = run_model( net_pred, is_train=3, data_loader=data_loader[act], opt=opt)
        ...
        log.save_npy_log(opt, value=out_seq, is_create=is_create, file_name=act) # 编写npy
        is_create = False
...
def run_model (net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        out_seq = p3d_out_4.cpu().data.numpy()
        ...
        if is_train == 0:  # 训练
            ...
            # loss_p3d_4 = torch.mean(torch.norm(p3d_out_all_4 - p3d_sup_4, dim=3))  # [32, 10, 22, 3]
            ...
            loss_total = 0
            for t in range(T):
                loss_part = torch.mean(torch.norm(p3d_out_all_4[:, t, :, :] - p3d_sup_4[:, t, :, :], dim=2)) * loss_weight[t] / total
                loss_total += loss_part
            loss_p3d_4 = loss_total / T
