...
def eval(opt):
    ...
    for act in acts:
        ...
        data_loader[act] = DataLoader(dataset, batch_size=opt.test_batch_size, shuffle=False, num_workers=0,
                                      pin_memory=True)
        # log.save_npy_log(opt, opt.truth_npy, value=data_loader[act], is_create=is_create, file_name=act)  # 编写实际输出npy
        # is_create = False
    # do test
    avg_ret_log = []
    is_create = True
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        if is_train == 0:  # 训练
            ...
            for t in range(T):
                ...
                # weight = 2 * (t + 1)
                # weight = math.exp(t / T)
                # weight = math.tanh((T - t) / T)
                # weight =h math.tanh(t / T)
                # weight = 1 / (1 + math.exp(-( t - 5)))
                # weight = 1 / (1 + math.exp(-(t - 5)))
                loss_weight.append(weight)
                total += weight
            loss_total = 0
            for t in range(T):
                loss_part = torch.mean(torch.norm(p3d_out_all_4[:, t, :, :] - p3d_sup_4[:, t, :, :], dim=2)) * \
                            loss_weight[t] / total
